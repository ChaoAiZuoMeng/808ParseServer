...
//			 props.put("group.id", GROUPID);
		...
		/*
		 * latest：从最新的偏移量开始拉取。 earliest：从最早的偏移量开始拉取。默认
		 */
	...
	private static Properties setProducerProperties(Object serializer) {
		...
		props.put("bootstrap.servers", BOOTSTRAP);
		/*
		 * 用来做应答 0: 客户端发送数据到kafka，不等待集群应答 1: 客户端发送数据到kafka，等待leader应答，不等待follower的应答 -1:
		 * 客户端发送数据到kafka，leader 等待follower应答，leader再向client应答 all: 同-1
		 * 想要跟高的吞吐量，设置为异步、ack = 0; 想要数据不丢失，设置为同步、ack = -1\all
		 */
		props.put("acks", "-1");
		props.put("retries", 0);
...
//			logger.info("以上数据发送成功 ---");
	...
	/*
	 * if null return false or not null return true
	 */
